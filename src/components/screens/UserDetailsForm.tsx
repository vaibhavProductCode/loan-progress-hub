...
import { ArrowLeft, ArrowRight, User, Building2, Home, DollarSign } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { Input } from '@/components/ui/input';
import { Label } from '@/components/ui/label';
import { useLoan } from '@/contexts/LoanContext';

type FormStep = 'personal' | 'employment' | 'loan';
...
export function UserDetailsForm() {
  ...
  const { currentApplicationId, applications, updateUserDetails } = useLoan();

  const currentIndex = currentStep === 'personal' ? 0 : currentStep === 'employment' ? 1 : 2;
  const totalSteps = 3;
  ...
  const handleNext = () => {
    if (currentStep === 'personal') {
      ...
      // Save user details to application and navigate to document upload
      if (currentApplicationId) {
        updateUserDetails(currentApplicationId, {
      ...
      case 'personal':
        return (
          <div className="space-y-4">
            ...
            <div><Label htmlFor="address">Address</Label><Input id="address" placeholder="Enter your current address" value={formData.address} onChange={(e) => handleInputChange('address', e.target.value)} className="mt-1.5" /></div>
          </div>
        );
      case 'employment':
        return (
  ...
  return (
    <div className="min-h-screen flex flex-col px-6 py-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <div>
          <p className="text-sm text-muted-foreground">Step {currentIndex + 1} of {totalSteps}</p>
          <h1 className="text-xl font-semibold font-serif">
            {currentStep === 'personal' && 'Personal Details'}
            {currentStep === 'employment' && 'Employment Details'}
            {currentStep === 'loan' && 'Loan Details'}
          </h1>
        </div>
      </div>
      ...
      <div className="flex gap-2 mb-4">
        {[0, 1, 2].map((step) => (
          <div key={step} className={`flex-1 h-1.5 rounded-full transition-colors ${step <= currentIndex ? "bg-primary" : "bg-border"}`} />
        ))}
      </div>
